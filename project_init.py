...
def get_args():
    ...
    parser.add_argument('--project_name', type=str, default='HRVConformer', help='Name of the project')
    ...
    parser.add_argument(
        '--num_run', type=int, default=1,
        help='Run-count metadata for external orchestration; this entry point executes one run',
    )
    ...
    parser.add_argument(
        '--data_root',
        type=str,
        default=os.environ.get('HRVCONFORMER_DATA_ROOT'),
        help=(
            'Directory containing the preprocessed <window_length>mins folders. '
            'Can also be set with HRVCONFORMER_DATA_ROOT.'
        ),
    )
    
    parser.add_argument('--wandb_enable', action='store_true', help='Enable Weights & Biases logging')
    parser.add_argument('--save_model', action='store_true', help='Save periodic checkpoints in addition to the selected model')
    
    parser.add_argument('--model_name', type=str, default='HRVConformer', help='Name of the model')
    parser.add_argument('--input_dim', type=int, default=1200, help='Input dimension for the model')
    parser.add_argument('--patch_size', type=int, default=100, help='Patch size in samples (25 s at 4 Hz)')
    ...
    parser.add_argument('--fcn_head_kernel_size', type=int, default=11, help='Kernel size for FCN head')
    ...
    parser.add_argument('--resume', type=str, default='', metavar='CHECKPOINT', help='Resume training from a checkpoint path')
    ...
    parser.add_argument('--label_smoothing', type=float, default=0.2, help='Label smoothing factor for the loss function')
    ...
    parser.add_argument('--warmup_epoch', type=int, default=50, help='Number of warmup epochs for learning rate scheduler')
